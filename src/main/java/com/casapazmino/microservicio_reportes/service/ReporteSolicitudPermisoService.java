...
import com.casapazmino.microservicio_reportes.model.SolicitudPermiso.AprobacionDTO;
import com.casapazmino.microservicio_reportes.model.SolicitudPermiso.ReporteSolicitudPermisoRequest;
import com.casapazmino.microservicio_reportes.model.SolicitudPermiso.SolicitudPermisoDTO;
...
@Service
public class ReporteSolicitudPermisoService {

    // ===================== PDF =====================
    public byte[] generarReporteSolicitudPermisoPDF(ReporteSolicitudPermisoRequest request) {
        ...
        final String TITULO_REPORTE = "SOLICITUD DE PERMISO";
        ...
        try {
            ...
            final SolicitudPermisoDTO s = request.getSolicitud();
            ...
            addCeldaValor(info, fmtEcDateTime(s.getFecha_creacion()));
            ...
            // APELLIDOS (2 bloques visibles: label + valor que ocupa 3 columnas)
            ...
            // TIPO PERMISO
            addCeldaLabel(motivo, "TIPO PERMISO:", colorPrincipal);
            addCeldaValor(motivo, safe(s.getTipo_permiso()));
            ...
            String ini = buildInicioMotivo(s);
            String fin = buildFinMotivo(s);

            addCeldaLabel(motivo, "FECHA INICIO:", colorPrincipal);
            addCeldaValor(motivo, ini);

            addCeldaLabel(motivo, "FECHA FIN:", colorPrincipal);
            addCeldaValor(motivo, fin);

            // OBSERVACIÓN
            addCeldaLabel(motivo, "OBSERVACIÓN:", colorPrincipal);
            addCeldaValorColspan(motivo, safe(s.getDescripcion()), 3);
            ...
            throw new ReportBuildException("No se pudo generar SolicitudPermiso.pdf", e);
        } finally {
    ...
    /**
     * Formatea ISO a hora Ecuador (America/Guayaquil) en dd/MM/yyyy HH:mm:ss
     */
    ...
    /**
     * Formatea ISO a fecha Ecuador dd/MM/yyyy
     */
    ...
    private boolean esPermisoPorHoras(SolicitudPermisoDTO s) {
        return (s.getHora_inicio() != null && !s.getHora_inicio().trim().isEmpty())
                || (s.getHora_fin() != null && !s.getHora_fin().trim().isEmpty());
    }

    private String buildInicioMotivo(SolicitudPermisoDTO s) {
        String fecha = fmtEcDate(s.getFecha_inicio());
        if (!esPermisoPorHoras(s)) return fecha;

        String hi = (s.getHora_inicio() == null) ? "" : s.getHora_inicio().trim();
        return (fecha + " " + hi).trim();
    }

    private String buildFinMotivo(SolicitudPermisoDTO s) {
        String fecha = fmtEcDate(s.getFecha_final());
        if (!esPermisoPorHoras(s)) return fecha;

        String hf = (s.getHora_fin() == null) ? "" : s.getHora_fin().trim();
        return (fecha + " " + hf).trim();
    }
