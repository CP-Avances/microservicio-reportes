...
public class ReporteConsolidadoSolicitudPermisoService {
    ...
    public byte[] generarReporteSolicitudesPermisosPDF(ReporteSolicitudesPermisosRequest request) {
        ...
        try {
            ...
            writer.setPageEvent(new ConfiguracionPaginaPDF(safe(request.getUsuario()),
                    safe(request.getFraseMarcaAgua()), safe(request.getColorPrincipal())));
            document.open();
            ...
            document.add(
                    ReporteUtil.crearTituloReporte("REPORTE DE SOLICITUDES DE PERMISOS - USUARIOS " + tipoUsuarios));
            document.add(ReporteUtil.crearTituloPeriodo(
                    "PERIODO DEL: " + safe(request.getFechaDesde()) + " AL " + safe(request.getFechaHasta())));
            ...
            int totalSolicitudes = grupos.stream().mapToInt(g -> g.solicitudes == null ? 0 : g.solicitudes.size())
                    .sum();
            ...
            PdfPCell celdaTitulo = new PdfPCell(
                    new Phrase("LISTA DE SOLICITUDES", ReporteUtil.fuenteEncabezadoTablaData()));
            ...
            PdfPCell celdaContador = new PdfPCell(
                    new Phrase("N° Registros: " + totalSolicitudes, ReporteUtil.fuenteEncabezadoTablaData()));
            ...
            for (GrupoEmpleadoReporte emp : grupos) {
                ...
                for (GrupoSolicitudReporte sol : emp.solicitudes) {
                    ...
                    contenido.addElement(
                            crearTablaCabeceraSolicitudPDF(
                                    sol,
                                    colorPrincipal,
                                    colorSecundario,
                                    fondo));

                    if (sol.aprobaciones != null &&
                            !sol.aprobaciones.isEmpty()) {
                        contenido.addElement(
                                crearTablaHistorialSolicitudPDF(
                                        sol,
                                        colorSecundario));
                    }
    ...
    public byte[] generarReporteSolicitudesPermisosExcel(ReporteSolicitudesPermisosRequest request) {
        ...
        final String[] HEADERS = {
                "ITEM", "IDENTIFICACIÓN", "CÓDIGO", "APELLIDO NOMBRE", "CIUDAD", "SUCURSAL", "RÉGIMEN", "DEPARTAMENTO",
                "CARGO",
                "CÓDIGO SOLICITUD", "TIPO PERMISO", "FECHA SOLICITUD", "FECHA INICIO", "FECHA FIN", "HORA INICIO",
                "TIMBRE INICIO",
                "HORA FIN", "TIMBRE FIN", "DÍAS", "DURACIÓN", "AUTORIZADO", "ESTADO SOLICITUD", "PASO",
                "DEPARTAMENTO APROBACIÓN",
                "AUTORIZA", "ESTADO FLUJO", "FECHA AUTORIZACIÓN", "OBSERVACIÓN"
        };
        final int[] ANCHOS = { 10, 20, 15, 28, 18, 24, 24, 28, 22, 18, 28, 22, 18, 18, 16, 18, 16, 18, 10, 14, 14, 20,
                10, 28, 28, 18, 22, 30 };
        ...
        try (XSSFWorkbook libro = new XSSFWorkbook(); ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
            ...
            for (int fila = 0; fila <= 4; fila++)
                UtilExcel.combinarCeldas(hoja, fila, fila, 1, HEADERS.length - 1);
            ...
            UtilExcel.establecerTexto(hoja, 1, 1,
                    "REPORTE DE SOLICITUDES DE PERMISOS - USUARIOS " + obtenerTextoUsuarios(request.getUsuarios()),
                    estiloTitulo);
            UtilExcel.establecerTexto(hoja, 2, 1,
                    "PERIODO DEL REPORTE: " + safe(request.getFechaDesde()) + " AL " + safe(request.getFechaHasta()),
                    estiloTitulo);

            Row encabezado = UtilExcel.asegurarFila(hoja, FILA_ENCABEZADO);
            for (int columna = 0; columna < HEADERS.length; columna++)
                UtilExcel.establecerTexto(encabezado, columna, HEADERS[columna], null);
            ...
            for (GrupoEmpleadoReporte empleado : grupos) {
                for (GrupoSolicitudReporte solicitud : empleado.solicitudes) {
                    List<AprobacionSolicitudReporte> aprobaciones = solicitud.aprobaciones == null
                            || solicitud.aprobaciones.isEmpty()
                    ...
                    for (AprobacionSolicitudReporte aprobacion : aprobaciones) {
                        ...
                        UtilExcel.establecerTexto(fila, columna++,
                                permisoPorHoras ? formatearHora(solicitud.horaInicio) : "", null);
                        UtilExcel.establecerTexto(fila, columna++,
                                permisoPorHoras ? formatearHora(solicitud.timbreInicioPermiso) : "", null);
                        UtilExcel.establecerTexto(fila, columna++,
                                permisoPorHoras ? formatearHora(solicitud.horaFin) : "", null);
                        UtilExcel.establecerTexto(fila, columna++,
                                permisoPorHoras ? formatearHora(solicitud.timbreFinPermiso) : "", null);
                        UtilExcel.establecerTexto(fila, columna++, formatearDias(solicitud.dias), null);
                        UtilExcel.establecerTexto(fila, columna++,
                                permisoPorHoras ? formatearDuracion(solicitud.minutosTotales) : "", null);
                        ...
                        UtilExcel.establecerTexto(fila, columna++,
                                aprobacion == null || aprobacion.ordenPaso == null ? ""
                                        : String.valueOf(aprobacion.ordenPaso),
                                null);
                        UtilExcel.establecerTexto(fila, columna++,
                                aprobacion == null ? "" : safe(aprobacion.departamentoAprobacion), null);
                        UtilExcel.establecerTexto(fila, columna++, aprobacion == null ? "" : safe(aprobacion.autoriza),
                                null);
                        UtilExcel.establecerTexto(fila, columna++,
                                aprobacion == null ? "" : safe(aprobacion.estadoFlujo), null);
                        UtilExcel.establecerTexto(fila, columna++,
                                aprobacion == null ? "" : formatFechaHora(aprobacion.fechaAutorizacion), null);
                        UtilExcel.establecerTexto(fila, columna, aprobacion == null ? "" : safe(aprobacion.observacion),
                                null);
                    }
                }
            }
            ...
            UtilExcel.aplicarEstiloARegion(hoja, FILA_ENCABEZADO, FILA_ENCABEZADO, 0, HEADERS.length - 1, estiloCentro,
                    true);

            if (ultimaFila > FILA_ENCABEZADO) {
                UtilExcel.aplicarEstiloARegion(hoja, FILA_ENCABEZADO + 1, ultimaFila, 0, HEADERS.length - 1,
                        estiloCentro, true);
                int[] columnasIzquierda = { 3, 5, 6, 7, 8, 10, 23, 24, 27 };
                for (int columna : columnasIzquierda)
                    UtilExcel.aplicarEstiloARegion(hoja, FILA_ENCABEZADO + 1, ultimaFila, columna, columna,
                            estiloIzquierda, true);

                boolean[] filtros = new boolean[HEADERS.length];
                Arrays.fill(filtros, true);
                filtros[0] = false;
                UtilExcel.crearTablaEstilizada(hoja, "SolicitudesPermisosReporteTabla", FILA_ENCABEZADO, 0, ultimaFila,
                        HEADERS.length - 1, true, filtros);
            }
    ...
    private PdfPTable crearTablaCabeceraSolicitudPDF(GrupoSolicitudReporte sol, Color colorPrincipal,
            Color colorSecundario, Color fondo) throws DocumentException {
        ...
        tabla.addCell(
                ReporteUtil.crearCelda("TIPO PERMISO", ReporteUtil.fuenteEncabezadoTablaData(), colorPrincipal, 2, 1));
        tabla.addCell(ReporteUtil.crearCelda("FECHA SOLICITUD", ReporteUtil.fuenteEncabezadoTablaData(), colorPrincipal,
                2, 1));
        ...
        tabla.addCell(
                ReporteUtil.crearCelda("AUTORIZADO", ReporteUtil.fuenteEncabezadoTablaData(), colorPrincipal, 2, 1));
        tabla.addCell(ReporteUtil.crearCelda("ESTADO SOLICITUD", ReporteUtil.fuenteEncabezadoTablaData(),
                colorPrincipal, 2, 1));
    ...
    private PdfPTable crearTablaHistorialSolicitudPDF(GrupoSolicitudReporte sol, Color colorSecundario)
        ...
        tabla.addCell(
                ReporteUtil.crearCelda("FECHA AUTORIZACIÓN", ReporteUtil.fuenteEncabezadoTablaData(), colorSecundario));
    ...
    private String formatearDias(Double dias) {
        if (dias == null)
            return "0";
        if (dias.doubleValue() == Math.rint(dias.doubleValue()))
