...
public class ReporteTimbresVirtualesMovilService {
    ...
    public byte[] generarReportePDF(ReporteTimbresVirtualesMovilRequest request) {
        ...
        final String TITULO_REPORTE = "TIMBRES (APLICACIÓN MOVIL) - "
                + (request.getOpcionBusqueda() == 1 ? "ACTIVOS" : "INACTIVOS");
        final String TITULO_PERIODO = "PERIODO DEL: " + request.getPeriodo().getInicio() + " AL "
    ...
    public byte[] generarReporteXLSX(ReporteTimbresVirtualesMovilRequest request) {
        ...
        try (XSSFWorkbook libro = new XSSFWorkbook();
            ...
            // 2) Merges cabecera (5 filas): con disp → B..R (1..17) | sin disp → B..P
            // (1..15)
            ...
            UtilExcel.establecerTexto(hoja, 1, 1, UtilExcel.aMayusculasSeguras("LISTA DE TIMBRES VIRTUALES MOVIL"),
                    estiloTitulo);
            ...
            org.apache.poi.ss.usermodel.Font fuenteLink =
                    libro.createFont();

            fuenteLink.setColor(
                    IndexedColors.BLUE.getIndex());

            fuenteLink.setUnderline(
                    org.apache.poi.ss.usermodel.Font.U_SINGLE);
