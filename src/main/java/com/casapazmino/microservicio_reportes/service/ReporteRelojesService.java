...
public class ReporteRelojesService {
    ...
    // =========================
    // XLSX (diseño legacy del front)
    // =========================
    public byte[] generarReporteXLSX(ReporteRelojesRequest request) {
        try (XSSFWorkbook libro = new XSSFWorkbook();
                ByteArrayOutputStream baos = new ByteArrayOutputStream()) {

            XSSFSheet hoja = libro.createSheet("Relojes");

            // 1) Logo estándar A1:B5
            byte[] logo = UtilExcel.decodificarImagenBase64(request.getLogoBase64());
            UtilExcel.insertarLogoEstandar(libro, hoja, logo); // A1:B5

            // 2) Merges B1:U1 ... B5:U5 (21 columnas: A..U)
            for (int r = 0; r < 5; r++) {
                UtilExcel.combinarCeldas(hoja, r, r, 1, 20); // B(1) .. U(20)
            }

            // 3) Títulos
            CellStyle estiloTitulo = ConfiguracionExcel.crearEstiloTitulo(libro);
            UtilExcel.establecerTexto(hoja, 0, 1, UtilExcel.aMayusculasSeguras(request.getEmpresa()), estiloTitulo);
            UtilExcel.establecerTexto(hoja, 1, 1, "LISTA DE RELOJES", estiloTitulo);

            // 4) Encabezados + anchos (fila 6 → idx 5)
            final int filaEncabezado = 5;
            String[] encabezados = {
                    "ITEM", "ID", "CODIGO", "NOMBRE", "IP", "PUERTO", "CONTRASEÑA", "MARCA", "MODELO", "SERIE",
                    "ID_FABRICACION", "FABRICANTE", "MAC", "TIPO CONEXION", "ID SUCURSAL", "ID DEPARTAMENTO",
            ...
            // 7) Tabla estilizada (A6:U n), zebra y AutoFilter
            if (ultimaFila >= filaDatosInicio) {
                boolean[] filtros = new boolean[encabezados.length];
                for (int i = 0; i < filtros.length; i++)
                    filtros[i] = (i != 0); // ITEM sin filtro
