...
@RestController
@RequestMapping("/api/reporte/planificacion")
    ...
    public ResponseEntity<byte[]> generarReportePlanificacion(@RequestBody ReportePlanificacionRequest request) {
        try {
            ...
            return ResponseEntity.ok()
                    ...
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=PlanificacionHoraria.pdf")
    ...
    public ResponseEntity<byte[]> generarReportePlanificacionXLSX(@RequestBody ReportePlanificacionRequest request) {
        try {
            ...
            return ResponseEntity.ok()
                    ...
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=PlanificacionHoraria.xlsx")
                    .header(HttpHeaders.CACHE_CONTROL, "no-store")
                    .header(HttpHeaders.PRAGMA, "no-cache")
                    .header(HttpHeaders.EXPIRES, "0")
                    .body(bin);
